...
public class MultiThreadServer {
	...
	public static void main(String[] args) {
		...
		try (ServerSocket serverSocket = new ServerSocket(5000)) {
			...
			// 클라이언트 대기 중 연결 요청 받으면 소켓 객체를 생성하는 메서드(클라이언트와 연결된 상태)
			Socket socket = serverSocket.accept(); 
			System.out.println("----- client connected -----");
			
			// 클라이언트와 통신을 위한 스트림을 설정해야 함 (대상 소켓을 얻었기 때문)
			BufferedReader readerStream =
					new BufferedReader(new InputStreamReader(socket.getInputStream()));
			
			PrintWriter writerStream =
					new PrintWriter(socket.getOutputStream(), true); // auto flush 기능
	...
	// 클라이언트로부터 데이터를 읽는 스레드 분리 ( 외부호출 필요 x -> 캡슐화 )
		...
		Thread writerThread = new Thread(() -> {
			try {
				...
				while( (severMessage = keyboardReader.readLine()) != null ) {
					printWriter.println(severMessage);
					printWriter.flush();
				}
			} catch (Exception e) {
				e.printStackTrace();
