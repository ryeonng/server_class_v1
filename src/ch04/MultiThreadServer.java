...
public class MultiThreadServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 클라이언트로 데이터를 보낼 출력 스트림 필요

			// 서버 측에서 키보드 입력을 받기 위한 입력 스트림 필요
			...
			Thread readThread = new Thread(() -> {
				...
				try {
					...
					while ((clientMessage = socketReader.readLine()) != null) {
						System.out.println("서버 측 콘솔 : " + clientMessage);
					}
				} catch (IOException e) {
					e.printStackTrace();
				}
			...
			// Thread.join 메서드는 하나의 스레드가 종료될 때 까지 기다리도록 하는 기능을 제공한다.
			readThread.join();
			writeThread.join();
